...
for i in range(17):
	...
	temp = temp/temp.mean()	
	SAP_FLUX_err_temp[i] = SAP_FLUX_err_temp[i] / SAP_FLUX_temp[i] * temp
...
SAP_FLUX_err = np.asarray(SAP_FLUX_err_temp)
...
SAP_FLUX_err = np.ma.concatenate([SAP_FLUX_err[0],SAP_FLUX_err[1],SAP_FLUX_err[2],SAP_FLUX_err[3],SAP_FLUX_err[4],SAP_FLUX_err[5],SAP_FLUX_err[6],SAP_FLUX_err[7],SAP_FLUX_err[8],SAP_FLUX_err[9],SAP_FLUX_err[10],SAP_FLUX_err[11],SAP_FLUX_err[12],SAP_FLUX_err[13],SAP_FLUX_err[14],SAP_FLUX_err[15],SAP_FLUX_err[16]])
...
SAP_FLUX_err = np.ma.masked_invalid(SAP_FLUX_err)
...
planet_period2 = planet_period/1.3575656
orbital_frequency2 = 1./planet_period2
left_boundary = (first_transit - transit_duration) % planet_period2
right_boundary = (first_transit + transit_duration) % planet_period2
sec_transit_left = (first_transit - planet_period2/2. + transit_duration) % planet_period2
sec_transit_right = (first_transit - planet_period2/2. - transit_duration) % planet_period2
time_temp = np.ma.mod(time,planet_period2)
...
#time_temp = np.ma.masked_inside(time_temp, sec_transit_right, sec_transit_left)
#time_temp = np.ma.masked_inside(time_temp, left_boundary, planet_period2)
...
frequency, power = LombScargle(time, PDCSAP_FLUX, PDCSAP_FLUX_err).autopower(minimum_frequency=10.**(-0.05)*orbital_frequency2, maximum_frequency=10.**(0.8)*orbital_frequency2)

print 'freq lenght:', len(frequency)

#Phase folding for illustrative purposes
##centering##
offset = ((first_transit%planet_period2) - planet_period2/2.)

time = np.ma.mod(time - offset,planet_period2)
...
ax2.set_xlim([frequency.min()/orbital_frequency2,frequency.max()/orbital_frequency2])
ax2.set_ylim([6.6e-5,0.7])
ax2.plot(frequency/orbital_frequency2, power)
ax2.set_yscale('log')
