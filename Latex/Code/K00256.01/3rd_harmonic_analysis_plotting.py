...
if LS_idx == 0:
	...
	for i in range(1,13):
		...
		if i == 10:
			...
			Ntemp = 30000.
		print Ntemp
		f = 2.*np.pi*np.logspace(-0.05, 0.8, Ntemp)*orbital_frequency
		power, z = lomb_scargle(time, PDCSAP_FLUX, PDCSAP_FLUX_err, f, significance = [1.-0.682689492,1.-0.999999426697])
		print z
		i=i-1
		axes[i].loglog(f/orbital_frequency/2./np.pi, power)
		axes[i].set_ylim([6.6e-5,0.7])
		axes[i].plot([fmin/orbital_frequency,fmax/orbital_frequency], [z[0],z[0]])
		axes[i].plot([fmin/orbital_frequency,fmax/orbital_frequency], [z[1],z[1]])
		axes[i].set_xlim([fmin/orbital_frequency,fmax/orbital_frequency])
		if i == 9 or i == 10 or i == 11:
			axes[i].set_xticks([1.,2.,3.,4.,5.,6.])
			axes[i].set_xticklabels([1,2,3,4,5,6])
			if i == 10:
				axes[i].set_xlabel('Frequency (1/orbital period)')
		if i%3==0:
			axes[i].set_ylabel('Power')
			axes[i].set_yticks([z[0],z[1]])
			axes[i].set_yticklabels([r'$1\sigma$',r'$5\sigma$'])	
		else:
			axes[i].set_yticks([])	
plt.show()
